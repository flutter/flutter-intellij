...
public class WidgetPerfLinter {
  ...
  public CompletableFuture<ArrayList<PerfTip>> getTipsFor(Set<TextEditor> textEditors) {
    ...
    // Widget tree inspection requires InspectorService (not restored).
    // Without a widget tree, tips that require ancestor matching are skipped.
    return perfProvider.getWidgetTree().thenApplyAsync((ignored) -> {
      // getWidgetTree() currently returns null (InspectorService not available).
      // Return matches without tree context.
