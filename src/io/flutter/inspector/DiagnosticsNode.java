...
public class DiagnosticsNode {
  ...
  public SourceLocation getCreationLocation() {
    if (location != null) {
      return location;
    }
    ...
    location = new SourceLocation(json.getAsJsonObject("creationLocation"), null);
    return location;
  }
  ...
  public CompletableFuture<ArrayList<DiagnosticsNode>> getProperties() {
    ...
    return properties.thenApplyAsync((ArrayList<DiagnosticsNode> nodes) -> {
      ...
      final SourceLocation creationLocation = getCreationLocation();
      if (creationLocation != null) {
        final ArrayList<SourceLocation> parameterLocations = creationLocation.getParameterLocations();
        if (parameterLocations != null) {
          final Map<String, SourceLocation> names = new HashMap<>();
          for (SourceLocation location : parameterLocations) {
          ...
          for (DiagnosticsNode node : nodes) {
            ...
            if (name != null) {
              final SourceLocation parameterLocation = names.get(name);
  ...
  private void setCreationLocation(SourceLocation location) {
