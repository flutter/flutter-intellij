...
public class FlutterPerformanceView implements Disposable {
  ...
  private void addPerformanceViewContent(FlutterApp app, ToolWindow toolWindow) {
    ...
    content.setIcon(FlutterIcons.Flutter);
    ...
    openDevtools.setListener((linkLabel, data) -> {
      AsyncUtils.whenCompleteUiThread(DevToolsService.getInstance(app.getProject()).getDevToolsInstance(), (instance, ex) -> {
        ...
        io.flutter.sdk.FlutterSdk flutterSdk = io.flutter.sdk.FlutterSdk.getFlutterSdk(app.getProject());
    ...
    final boolean isInProfileMode = app.getMode().isProfiling() || app.getLaunchMode() == FlutterLaunchMode.PROFILE;
    ...
    if (debugConnectionAvailable) {
      ...
      // Create the FPS and widget recount areas.
