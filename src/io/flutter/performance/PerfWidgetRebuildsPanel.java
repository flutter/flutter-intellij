...
public class PerfWidgetRebuildsPanel extends JBPanel<PerfWidgetRebuildsPanel> {
  ...
  PerfWidgetRebuildsPanel(@NotNull FlutterApp app, @NotNull Disposable parentDisposable) {
    ...
    final boolean isInProfileMode = app.getMode().isProfiling() || app.getLaunchMode() == io.flutter.run.FlutterLaunchMode.PROFILE;
    if (isInProfileMode){
    ...
    app.hasServiceExtension(ServiceExtensions.trackRebuildWidgets.getExtension(), trackRebuildsCheckbox::setEnabled);

    if (ENABLE_TRACK_REPAINTS) {
      app.hasServiceExtension(ServiceExtensions.trackRepaintWidgets.getExtension(), trackRepaintsCheckbox::setEnabled);
    }
